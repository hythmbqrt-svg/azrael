...
// ── Bot message styling ──────────────────────────────────────────────────────
function decorateBotMessage(message) {
  if (typeof message !== "string" || !message.trim()) return message;
  return `⛧ 𖤐 عزرائيل 𖤐 ⛧\n${message}\n𖤐 ⛧`;
}

function createCommandApi(api) {
  return new Proxy(api, {
    get(target, property, receiver) {
      if (property !== "sendMessage") return Reflect.get(target, property, receiver);
      return (message, threadID, ...rest) =>
        target.sendMessage(decorateBotMessage(message), threadID, ...rest);
    },
  });
}
...
async function handleMessage(api, event, commands) {
  ...
  try {
    await cmd.execute({ api: createCommandApi(api), event: { ...event, isGroup }, args, commands, mutedThreads, lockedThreads });
  } catch (e) {
