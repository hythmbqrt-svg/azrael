...
module.exports = {
  ...
  async execute({ api, event, args, commands }) {
    ...
    if (args[0]) {
      ...
      if (!cmd) {
        return api.sendMessage(`☠️ الأمر "${name}" غير موجود في الظلال.\nاكتب ${prefix}help لاستدعاء القائمة.`, event.threadID);
      }
      const lines = [
        `⛧ تفاصيل الطقس: ${prefix}${cmd.name}`,
        `𖤐 الوصف       : ${cmd.description}`,
        `☠️ الفئة       : ${cmd.category || "General"}`,
        `⚔️ الاستخدام   : ${prefix}${cmd.usage || cmd.name}`,
      ];
      if (cmd.aliases?.length) {
        lines.push(`⛓️ الاختصارات : ${cmd.aliases.map(a => prefix + a).join("  ")}`);
      }
      if (cmd.adminOnly)  lines.push(`🔒 يتطلب ختم المشرف`);
      if (cmd.groupOnly)  lines.push(`☠️ للمجموعات فقط`);
    ...
    let msg = `╔══════ ⛧ 𖤐 ${config.bot.name} 𖤐 ⛧ ══════╗\n`;
    msg += `║        كتاب الأوامر المحرّمة        ║\n║\n`;
    ...
    msg += `╚══ اكتب ${prefix}help <أمر> لفتح ختم التفاصيل ══╝`;
